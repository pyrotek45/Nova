...
impl Disassembler {
    ...
    pub fn dis(
        ...
        while let Some(code) = input.next() {
            match code {
                Code::RET => {
                    self.out("Return");
                    if self.depth >= 1 {
                ...
                Code::BYTE => {
                    let int = input.next().unwrap() as i64;
                    self.out(&format!("Push Integer {}", int))
                }
                Code::FLOAT => {
                ...
                Code::ID => {
                    let index = u16::from_ne_bytes([input.next().unwrap(), input.next().unwrap()]);
